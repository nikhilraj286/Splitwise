...
class ViewGroup extends React.Component {
  ...
  componentDidUpdate = async (prevProps, prevState) => {
    if (
      (prevState.commentCreated === false &&
        this.state.commentCreated === true) ||
      (prevState.commentDeleted === false &&
        this.state.commentDeleted === true) ||
      (prevState.expenseCreated === false &&
        this.state.expenseCreated === true) ||
  ...
  render = () => {
    ...
    if (this.state.groups) {
      ...
      //     ? this.props.getAllUsersNamesDetails
      //     : null;
      let allUsers = this.state.getAllUsersNames
        ? this.state.getAllUsersNames
        : null;
      temp.forEach((item) => {
        if (!item.has_invite) {
              ...
              <div className="row" style={{ margin: "20px 0" }}>
                ...
                <div className="col-8" style={{ paddingLeft: "0" }}>
                  <div style={{ marginBottom: "5px" }}>
                    {allUsers && allUsers[item.user_id]
                      ? allUsers[item.user_id].name
                      : ""}
                  </div>
      ...
      let user_id = localStorage.getItem("userProfile")
      ...
      if (expense_sum[user_id] === 0) {
        leaveGroupClass = (
          <div>
            ...
            <div style={{ margin: "10px", fontSize: "13px", color: "#444" }}>
        ...
        leaveGroupClass = (
          <div>
            <button
              ...
              title="Clear the dues to leave this group"
              disabled
            >
              Leave Group
            </button>
            <div style={{ margin: "10px", fontSize: "13px", color: "#444" }}>
              Clear the dues to leave this group
            </div>
            <hr></hr>
          </div>
        );
    ...
    // let all_users = this.props.getAllUsersNamesDetails
    //   ? this.props.getAllUsersNamesDetails
    //   : null;

    let all_users = this.state.getAllUsersNames
      ? this.state.getAllUsersNames
      : null;
        ...
        if (comments.length > 0) {
          comments.forEach((comment) => {
            let commentDeleteButton =
              comment.user_id === user_id ? <i class="fa fa-trash"></i> : null;
            comment_disp.push(
              <div
                className="comment_div row"
                key={comment._id}
                id={comment._id}
                style={{
              ...
              >
                ...
                >
                  ...
                  <div style={{ whiteSpace: "pre-wrap", paddingRight: "60px" }}>
                    {comment.comment}
                  </div>
                </div>
                <div
                  className="col-1 delete-comment"
                  style={{
                    display: "flex",
                    flexDirection: "column",
                    justifyContent: "center",
        ...
        let commentBody = (
          ...
          >
            ...
            <div
              ...
              style={{ fontSize: "14px", color: "white", margin: "10px 0" }}
              onClick={async (e) => {
        ...
        expenseList.push(
          <div>
              ...
              onClick={(e) => {
                if (document.getElementById(item.exp_id)) {
                  ...
                  if (comm.classList.contains("hidden")) {
                    document
                      .getElementById("e" + item.exp_id)
                      .classList.add("bg_gray");
                    comm.classList.remove("hidden");
                  } else {
                    document
                      .getElementById("e" + item.exp_id)
                      .classList.remove("bg_gray");
            ...
            >
              ...
              >
                <div className="expense-desc" style={{ display: "flex" }}>
                  <strong>{item.desc}</strong> {commentIcon}
                </div>
              </div>
              <div className="col-3">
                <div style={{ fontSize: "12px", color: "#777" }}>
                  <strong style={{ textTransform: "capitalize" }}>
                    {all_users ? all_users[item.paid_by].name : null}
                  </strong>{" "}
                  paid
                </div>
                <div>
                  <strong>
                    {currencyIcon} {item.amount}
                  </strong>
                </div>
              </div>
            </div>
              ...
              <div className="col-4">
                ...
                >
                  ...
                  >
                    ...
                    <Modal.Body>
                      <div>
                        Paid by <strong>you</strong> and{" "}
                        <strong>split equally</strong> among all
                      </div>
                      <hr />
                      <div className="row">
