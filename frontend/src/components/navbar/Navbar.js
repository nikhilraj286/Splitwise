...
class Navbar extends Component {
    ...
    render(){
        ...
        if(localStorage.getItem('userProfile')){
            navLogin = (
                <div style={{width:'100%'}}>
                    ...
                    <img id="logo" width="110px" alt="logo" style={{float:'left', margin:'7px 0'}} src="https://assets.splitwise.com/assets/core/logo-wordmark-horizontal-white-short-c309b91b96261a8a993563bdadcf22a89f00ebb260f4f04fd814c2249a6e05d4.svg"></img>
                    <ul className="nav navbar-nav navbar-right"  style={{flexDirection:'row', float:'right'}}>
                        {/* <button className="btn btn-primary btn-green" style={{paddingTop:'0px', paddingBottom:'0px', marginRight:'10px', maxHeight:'35px'}}><Link to="/userAccount" style={{color:'#fff', textDecoration:'none', fontWeight:'bold'}}> <img src={imageUser} style={{width:'20px'}} alt="userIcon"/> Your Account</Link></button> */}
                        <Link className="btn btn-green" to="/userAccount" style={{color:'#fff', textDecoration:'none', fontWeight:'bold', marginRight:'20px'}}><img src={imageUser} style={{width:'17px'}} alt="userIcon"/> Your Account</Link>
                        <a onClick = {()=>{
                            localStorage.clear();
                            this.props.logout();}} href="/" className="btn btn-orange" style={{paddingTop:'0px', paddingBottom:'0px', height:'35px'}}>
                            {/* <Link to="/" style={{color:'#fff', textDecoration:'none', fontWeight:'bold'}}>Log out</Link> */}
                            logout
                            </a>
                    </ul>
                </div>
            ...
            navLogin = (
                <div style={{width:'100%'}}>
                    <img id="logo" width="110px" alt="logo" style={{float:'left', margin:'7px 0'}} src="https://assets.splitwise.com/assets/core/logo-wordmark-horizontal-white-short-c309b91b96261a8a993563bdadcf22a89f00ebb260f4f04fd814c2249a6e05d4.svg"></img>
                    <ul className="nav navbar-nav navbar-right" style={{flexDirection:'row', float:'right'}}>
                        {/* <button className="btn btn-primary btn-green" style={{paddingTop:'0px', paddingBottom:'0px', maxHeight:'35px'}}><Link to="/login" style={{color:'#fff', textDecoration:'none', fontWeight:'bold'}}>Log in</Link></button> */}
                        <Link className="btn btn-green" to="/login" style={{color:'#fff', textDecoration:'none', fontWeight:'bold'}}>Log in</Link>
                        <p style={{margin:'5px 15px'}}>or</p>
                        {/* <button className="btn btn-primary btn-orange" style={{paddingTop:'0px', paddingBottom:'0px', maxHeight:'35px'}}><Link to="/signup" style={{color:'#fff', textDecoration:'none', fontWeight:'bold'}}>Sign up</Link></button> */}
                        <Link className="btn btn-orange" to="/signup" style={{color:'#fff', textDecoration:'none', fontWeight:'bold'}}>Sign up</Link>
                    </ul>
                </div>
            );
            // displayBackground = (
        ...
        // else {
        //     landing = <div style={{position:"absolute", width:'100%', marginTop:"100px"}}><img width="100%" src={imageHome} alt=""/></div>
        // }
        return(
            <div>
                {redirectVar}
...
//     return { logoutDetails: state.logoutDetails }
// }
