...
const updateUserHandler = async (msg, callback) => {
    res = {}
    let data = {}
    if(msg.email){data.email = msg.email}
    if(msg.full_name){data.full_name = msg.full_name}
    if(msg.phone){data.phone = msg.phone}
    if(msg.currency){data.currency = msg.currency}
    if(msg.time_zone){data.time_zone = msg.time_zone}
    if(msg.language){data.language = msg.language}
    if(msg.profile_picture){data.profile_picture = msg.profile_picture}
    try {
